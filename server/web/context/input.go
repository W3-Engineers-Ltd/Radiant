...
func (input *radiantInput) Bind(dest interface{}, key string) error {
	...
	if value.Kind() != reflect.Ptr {
		return errors.New("radiant: non-pointer passed to Bind: " + key)
	}
	value = value.Elem()
	if !value.CanSet() {
		return errors.New("radiant: non-settable variable passed to Bind: " + key)
	}
	typ := value.Type()
	...
	if !rv.IsValid() {
		return errors.New("radiant: reflect value is empty")
	}
	value.Set(rv)
	return nil
}

func (input *radiantInput) bind(key string, typ reflect.Type) reflect.Value {
	if input.Context.Request.Form == nil {
		input.Context.Request.ParseForm()
	}
	rv := reflect.Zero(typ)
...
func (input *radiantInput) bindValue(val string, typ reflect.Type) reflect.Value {
	rv := reflect.Zero(typ)
	switch typ.Kind() {
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		rv = input.bindInt(val, typ)
...
func (input *radiantInput) bindInt(val string, typ reflect.Type) reflect.Value {
...
func (input *radiantInput) bindUint(val string, typ reflect.Type) reflect.Value {
...
func (input *radiantInput) bindFloat(val string, typ reflect.Type) reflect.Value {
...
func (input *radiantInput) bindString(val string, typ reflect.Type) reflect.Value {
	return reflect.ValueOf(val)
}

func (input *radiantInput) bindBool(val string, typ reflect.Type) reflect.Value {
...
func (input *radiantInput) bindSlice(params *url.Values, key string, typ reflect.Type) reflect.Value {
	maxIndex := -1
	numNoIndex := 0
...
func (input *radiantInput) bindStruct(params *url.Values, key string, typ reflect.Type) reflect.Value {
...
func (input *radiantInput) bindPoint(key string, typ reflect.Type) reflect.Value {
	return input.bind(key, typ.Elem()).Addr()
}

func (input *radiantInput) bindMap(params *url.Values, key string, typ reflect.Type) reflect.Value {
	var (
