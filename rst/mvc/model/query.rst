...
ORM uses **QuerySeter** to organize queries. Every method that returns
**QuerySeter** will give you a new **QuerySeter** object.
...
Field combination orders are decided by the relationship of tables. For
example, ``User`` has a foreign key to ``Profile``, so if you want to
use ``Profile.Age`` as the condition, you have to use the expression
``Profile__Age``. Note that the separator is double under scores ``__``.
``Expr`` can also append operators at the end to execute related SQL.
For example, ``Profile__Age__gt`` represents condition query
``Profile.Age > 18``.

Comments below describe SQL statements that are similar to the expr, but
may not be the exact generated results.
...
-  `exact <#exact>`__ / `iexact <#iexact>`__ equal to
-  `contains <#contains>`__ / `icontains <#icontains>`__ contains
-  `gt / gte <#gt%20/%20gte>`__ greater than / greater than or equal to
-  `lt / lte <#lt%20/%20lte>`__ less than / less than or equal to
-  `startswith <#startswith>`__ / `istartswith <#istartswith>`__ starts
   with
-  `endswith <#endswith>`__ / `iendswith <#iendswith>`__ ends with
-  `in <#in>`__
-  `isnull <#isnull>`__
...
-  Every API call that returns **QuerySeter** will give you a new
   **QuerySeter** object. It won’t affect the previous object.

-  Advanced queries use ``Filter`` and ``Exclude`` to do conditional
   queries. There are two filter rules - contain and exclude
...
Use ``NOT`` to exclude condition Use ``AND`` to connect multiple
filters:
...
Same as ``distinct`` statement in sql, return only distinct (different)
values
...
Use a prepared statement to increase inserting speed with multiple
inserts.
...
All / Values / ValuesList / ValuesFlat will be limited by
`Limit <#limit>`__. 1000 lines by default.
...
The other fields of the object are set to the default value of the
field’s type.
...
**TODO**: doesn’t support recursive query. **RelatedSel** return Values
directly
...
The order of the result is same as the Fields order in the Model
definition.
...
Let’s see how to do a Relational Query by looking at `Model
Definition <orm.html>`__
...
While RegisterModel, ORM will create reverse relation for Post in User.
So it can query directly:
...
LoadRelated is used to load relation field of model. Including all
rel/reverse - one/many relation.
